import { useSite } from "@/contexts/SiteContext";
import { EditableText } from "./EditableText";
import { Button } from "@/components/ui/button";
import { ChevronDown } from "lucide-react";

export function HeroSection() {
  const { content, setContent, language, editMode } = useSite();
  const hero = content.hero;

  const updateHero = (field: string, value: string) => {
    setContent((prev) => ({
      ...prev,
      hero: {
        ...prev.hero,
        [field]: typeof prev.hero[field as keyof typeof prev.hero] === "object"
          ? { ...(prev.hero[field as keyof typeof prev.hero] as any), [language]: value }
          : value,
      },
    }));
  };

  return (
    <section className="relative min-h-screen flex items-center justify-center overflow-hidden">
      {/* Background image */}
      <div className="absolute inset-0">
        <img src={hero.image} alt="Hero" className="w-full h-full object-cover" />
        <div className="absolute inset-0 bg-gradient-to-b from-background/70 via-background/50 to-background" />
        <div className="absolute inset-0 bg-gradient-to-r from-background/80 to-transparent" />
      </div>

      {/* Content */}
      <div className="relative z-10 container mx-auto px-4 text-center">
        <div className="max-w-4xl mx-auto space-y-6">
          <EditableText
            value={hero.title[language]}
            onSave={(v) => updateHero("title", v)}
            as="h1"
            className="text-5xl md:text-7xl lg:text-8xl font-display font-bold leading-tight text-gradient animate-fade-in-up"
          />
          <EditableText
            value={hero.subtitle[language]}
            onSave={(v) => updateHero("subtitle", v)}
            as="p"
            className="text-xl md:text-2xl text-muted-foreground font-body max-w-2xl mx-auto animate-fade-in-up opacity-0"
            style={{ animationDelay: "200ms" } as any}
          />
          <div className="animate-fade-in-up opacity-0 pt-4" style={{ animationDelay: "400ms" }}>
            {editMode ? (
              <div className="flex flex-col items-center gap-2">
                <Button variant="hero" size="xl">
                  {hero.cta[language]}
                </Button>
                <input
                  type="text"
                  value={hero.ctaUrl}
                  onChange={(e) =>
                    setContent((prev) => ({ ...prev, hero: { ...prev.hero, ctaUrl: e.target.value } }))
                  }
                  className="bg-muted text-foreground border border-border rounded px-3 py-1 text-sm w-64 text-center"
                  placeholder="URL del botón"
                />
              </div>
            ) : (
              <a href={hero.ctaUrl}>
                <Button variant="hero" size="xl" className="animate-glow-pulse">
                  {hero.cta[language]}
                </Button>
              </a>
            )}
          </div>
        </div>
      </div>

      {/* Scroll indicator */}
      <div className="absolute bottom-8 left-1/2 -translate-x-1/2 animate-bounce">
        <ChevronDown className="h-8 w-8 text-primary" />
      </div>
    </section>
  );
}
